...
if st.button("▶️ Go"):
    ...
    elif not price_input.strip():
        st.error("Please enter a price to update.")
    else:
        try:
                ...
                if 'rows' in section:
                    ...
                    for row in section['rows'].values():
                        for seat in row['seats'].values():
                            ...
                            if price_only_mode:
                                # 🔧 Update only seats with 'av' status OR matched seats if range is given
